...
import { tasks, reportTypeLabels, statusConfig, priorityConfig } from "@/lib/mock-data";
        ...
        {myTasks.map((task, i) => {
          ...
          return (
            ...
            >
              <Link
                to={`/app/task/${task.id}`}
                className="flex items-center gap-4 p-4 glass-panel hover:border-primary/20 transition-all group"
              >
                <div className="flex-1 min-w-0">
                  <p className="font-medium">{task.title}</p>
                  <p className="text-xs text-muted-foreground mt-1">
                    {task.project} · {reportTypeLabels[task.reportType]}
                  </p>
                </div>
                <span className={cn("status-badge shrink-0", statusConfig[task.status].color)}>
